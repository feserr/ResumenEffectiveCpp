...
\begin{itemize}
  ...
  activase antes un retorno estaríamos dejando memoria por liberar. También 
  puede ocurrir si creamos y eliminamos objetos dentro de un bucle y en este 
  hay \textit{break} o \textit{goto}. No hay que olvidarse de las funciones que 
  pueden lanzar excepciones.
  \item Para ello existe \textit{unique\_ptr} y \textit{shared\_ptr}. Estos dos 
  ...
  \item Algo positivo de \textit{unique\_ptr} y \textit{shared\_ptr} es que 
  siguen 
...
\begin{itemize}
    ...
    \textit{unique\_ptr} o \textit{shared\_ptr} no sirven, por lo que tendrás 
    que 
    crear uno propio.
